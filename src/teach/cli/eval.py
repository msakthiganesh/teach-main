...
def main():
    ...
    # logger.info("Input files %d" % len(set(input_files)))
    # logger.info("Ouptut files %d" %  len(set(output_files)))
    # inp_f = list(input_files)
    # output_f = list(output_files)
    # for i in range(4):
    #     logger.info("Input file name %d : %s" % (i, inp_f[i]))
    #     logger.info("Output file name %d : %s" % (i, output_f[i]))
    ...
    edh_instance_files =  set(os.listdir(os.path.join(args.data_dir, input_subdir, args.split))) # Added this line to fix bug
